...
$searchedDrink1 = $_POST['search_key'];


$searchedDrink2 = explode(",", $searchedDrink1);
$searchedTerm = $searchedDrink2[0];
					...
					<div class="home_container">
						<div class="home_container_inner d-flex flex-column align-items-center justify-content-center">
							<div class="home_content text-center">
								...
								<div class="home_title"><h1><?php  echo $search_key." Search results";?></h1></div>
				...
				<div class="col-lg-8 order-lg-1 order-2 shows_list_col">
					<div class="shows_list_container">
						<ul class="shows_list">
							...
							<!-- Show -->
<?php
	$select= mysqli_query($server, "SELECT * FROM music WHERE music_name LIKE '%".$searchedTerm."%' OR artist	LIKE '%".$searchedTerm."%' OR genre LIKE '%".$searchedTerm."%'	OR file LIKE '%".$searchedTerm."%' ORDER BY  `music`.`music_name` DESC   ")  or die(mysql_error());


	//mysqli_query($server, "SELECT * FROM music WHERE music_name IN ($searchedTerm) || artist IN ($searchedTerm)|| genre IN ($searchedTerm) || file IN ($searchedTerm)  ORDER BY  `music`.`music_name` DESC   ")  or die(mysqli_error($server));		
	
//ABOVE CODE SORTS FETCHED ITEMS

 while($column=mysqli_fetch_array($select, MYSQLI_NUM))
 {
                                      	echo "
